...
'@
    ...
    function get_job_queue {
        ...
        if ($result.StatusCode -eq 200) {
            ...
            Throw [String]::Format("`n- FAIL, GET request failed to get job queue, statuscode {0} returned",$result.StatusCode)
        }
        $get_result=$result.Content | ConvertFrom-Json
        $get_member_array = $get_result.Members
        if ($get_member_array.count -gt 0) {
            Write-Verbose "- Complete job queue details for iDRAC $idrac_ip -`n"
            Start-Sleep 5
        } else {
            Throw "- WARNING, current iDRAC job queue is already cleared, no existing job IDs`n"
        }
    ...
    function delete_job_queue {
        Write-Verbose "- WARNING, clearing job queue for iDRAC $idrac_ip, this may take up to minute to complete depending on how many jobs are in the job queue`n"
        Start-Sleep 5
        ...
        if ($post_result.StatusCode -eq 200 -or $post_result.StatusCode -eq 200) {
            Write-Verbose "- PASS, POST command passed to successfully clear the job queue`n"
        } else {
            Throw [String]::Format("- FAIL, POST command failed to clear job queue, statuscode {0} returned. Detail error message: {1}",$post_result.StatusCode, $post_result)
        }
        Start-Sleep 10
        $uri = "https://$idrac_ip/redfish/v1/Managers/iDRAC.Embedded.1/Jobs"
        try {
        ...
        $get_member_array = $get_result.Members
        if ($get_member_array.count -gt 0) {
            Throw "- FAIL, job ID not successfully cleared, manually check iDRAC job queue`n"
        } else {
            Write-Verbose "- PASS, iDRAC job queue successfully cleared`n"
            return
        }
    }
